...
function Toolshome(props) {
    ...
    const getmasterstudentdetails = async (newdata) => {
        ...
        try {
            const response = await fetch('http://localhost:2025/api/getmasterstudentdetails', {
                method: 'GET',
                headers: {
                    "Content-Type": "application/json"
                }
    ...
    const gettingsheet2 = async (e) => {
        ...
        try {
          ...
          const json = await response.json();
          if (json.success) {
            alert('Successfully fetched new student data');
            const newdata = json.data;
    
            // Fetch the original student database
            const masterResponse = await fetch('http://localhost:2025/api/getmasterstudentdetails', {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
              },
            ...
            if (masterJson.success) {
                alert('Successfully fetched masterstudent data');
              const masterData = masterJson.data;
    
              // Update the master student data with new sheet data
              const updatedMasterData = masterData.map((student) => {
                const newStudentData = newdata.find((newStudent) => newStudent.Scholarid === student.Scholarid);
                return newStudentData
                  ? { ...student, FTE: newStudentData.FTE, FTESalary: newStudentData.FTESalary }
                  : student;
              });
    
              // Send updated data back to the server
             
              const updateResponse = await fetch('http://localhost:2025/api/updatingmasterstudentdetails', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                },
              ...
              if (updateJson.success) {
                alert('Successfully updated master student data');
              }
            }
          }
        } catch (error) {
    ...
    return (
        ...
        <div>
            ...
            <div className="main1">
                <h2>Create Master Database of students</h2>
            ...
            <div className="main1">
                <h2>Check eligibility of a student
                </h2>
                <div>
